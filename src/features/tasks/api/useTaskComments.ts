...
import {DBTaskComment, TaskComment} from '../types';
interface TaskCommentBase {
  path: string;
  data?: any;
}
interface TaskCommentPost extends TaskCommentBase {
  data: DBTaskComment;
}
...
export const taskCommentPostOptions = {
  ...
  mutationFn: async (mutation_data: TaskCommentPost) => {
    ...
    const {data: result} = await apiClient.post(`/comments`, data);
    return result;
  },
};
export const taskCommentPutOptions = {
  ...
  mutationFn: async (mutation_data: TaskCommentPut) => {
    ...
    const {data: result} = await apiClient.put(`/comments/${path}`, data);
    return result;
  },
};
...
export const useTaskComments = (task_id: string | undefined) => {
  ...
  const get = useQuery<Array<TaskComment>, APIError>({
    ...
    queryFn: async () => {
      //   const {data} = await apiClient.get<Array<TaskComment>>(`/comments/${task_id}`);
      return dummyData;
    },
