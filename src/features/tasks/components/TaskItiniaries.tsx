...
const sortByAscendingDate = (a: string, b: string) => {
  const dateA = moment(a, 'MMMM YYYY').toDate().getTime();
  const dateB = moment(b, 'MMMM YYYY').toDate().getTime();
  return dateA - dateB;
};
    ...
    select: (data) => {
      return data.reduce(
        (acc: Record<string, Taskitinerary[]>, itinerary: Taskitinerary) => {
          const date = moment(itinerary.due_date).format('MMMM YYYY');
          if (!acc[date]) {
            acc[date] = [];
          }
