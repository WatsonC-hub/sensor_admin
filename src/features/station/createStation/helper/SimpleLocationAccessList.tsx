...
import {SimpleLocationAccess} from '../controller/types';
...
const SimpleLocationAccessList = ({values, onRemove, onEdit}: Props) => {
  return (
    <List sx={{maxWidth: 360, bgcolor: 'background.paper'}}>
      {values.map((contact, index) => (
        <ListItem key={index} sx={{px: 1, width: '100%'}} disableGutters disablePadding>
          ...
          <ListItemText sx={{width: '100%'}} primary={contact.name} secondary={contact.type} />
